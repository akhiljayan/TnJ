...
/**
 * Description of uploaderHandler
 *
 * @author akhil
 */
class uploaderHandler {
     ...
     */
    public function handleUpload($request, $uploadDirectory, $name = null) {
        ...
        # non-chunked upload
        $basepath = $_SERVER['DOCUMENT_ROOT'] . $request->getBasePath();
        $folder = "galary-uploads";
        $target = join(DIRECTORY_SEPARATOR, array($basepath, $folder,$name));
        $type = $file['type'];
        $size = $file['size'];
        ...
        if ($target) {
            ...
            if (move_uploaded_file($file['tmp_name'], $target)) {
                ...
                return array('success' => true, "base64" => $base64,'target' => $target, 'fileName'=>$name, 'fileType'=>$type,'fileSize'=>$size);
            }
        } else {
